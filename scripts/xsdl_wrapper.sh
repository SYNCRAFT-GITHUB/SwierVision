# A wrapper for the popular script for IDEXScreen on Android over adb over USB.
# https://klipper.discourse.group/t/how-to-idexscreen-on-android-smart-phones/1196
#
# Keeps the process alive while the server is active
# to mimic default X's behaviour and keep the service happy

if  [ -f $PWD/launch_idexscreen.sh ]; then
	exec $PWD/launch_idexscreen.sh
elif [ ! -f $PWD/launch_idexscreen.sh ]; then
	echo "launch_idexscreen.sh does not exist"
	exit
fi
