...
class SwierVisionLoggingHandler(logging.handlers.RotatingFileHandler):
    def __init__(self, filename, **kwargs):
        super(SwierVisionLoggingHandler, self).__init__(filename, **kwargs)
        self.rollover_info = {
            'header': f"{'-' * 20}SwierVision Log Start{'-' * 20}",
            'version': f"SwierVision Version: {get_software_version()}",
    ...
    def doRollover(self):
        super(SwierVisionLoggingHandler, self).doRollover()
        self.log_start()
...
def setup_logging(log_file):
    ...
    fh = listener = None
    try:
        fh = SwierVisionLoggingHandler(log_file, maxBytes=4194304, backupCount=1)
        ...
        print(
            ...
            f"and SwierVision has Read/Write access to the folder.\n"
            f"{e}\n"
        )
    if listener is None:
