from sv_includes.host import Moonraker as M
...
class SwierVisionConfig:
    config = None
    configfile_name = "SwierVision.conf"
    ...
    def __init__(self, configfile, screen=None):
        ...
        host_json_path = os.path.join(os.getcwd(), "sv_includes", "dev-host.json")
    ...
    def create_translations(self):
        lang_path = os.path.join(swiervisiondir, "sv_includes", "locales")
        self.lang_list = [d for d in os.listdir(lang_path) if not os.path.isfile(os.path.join(lang_path, d))]
        self.lang_list.sort()
        for lng in self.lang_list:
            self.langs[lng] = gettext.translation('SwierVision', localedir=lang_path, languages=[lng], fallback=True)
                ...
                if key not in bools and key not in strs and key not in numbers:
                    ...
                    if key == "camera_url":
                        msg = (
                            ...
                            + "remove camera_url from SwierVision config file"
                        )
                    if remove:
    ...
    def _create_configurable_options(self, screen):
        ...
        self.configurable_options = [
            ...
            {"theme": {
                ...
                "value": "Industrial", "callback": screen.restart_sv, "options": [
            ...
            {"font_size": {
                ...
                "value": "medium", "callback": screen.restart_sv, "options": [
        ...
        t_path = os.path.join(swiervisiondir, 'styles')
    ...
    def get_config_file_location(self, file):
        # Passed config (-c) by default is ~/SwierVision.conf
        logging.info(f"Passed config (-c): {file}")
        if os.path.exists(file):
            return file

        file = os.path.join(swiervisiondir, self.configfile_name)
        if os.path.exists(file):
            return file
        file = os.path.join(swiervisiondir, self.configfile_name.lower())
        if os.path.exists(file):
            return file
